...
def solve_part_one_single(single: SingleInput):
    total_len = len(single.raw)
    string_stripped = single.raw.strip("\"")
    without_quote = string_stripped.replace(r'\"', "a")
    without_slash = without_quote.replace(r'\\', "a")
    without_hex = re.sub(r'\\x..', "a", without_slash)

    return total_len - len(without_hex)
